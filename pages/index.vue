...
export default {
 ...
 head() {
  return {
   title: this.index.seo.seo_title,
   meta: [
    {
     hid: `${this.index.id}-description`,
     name: "description",
     content: this.index.seo.seo_description,
    },
   ],
  };
 },
 async asyncData({ $axios }) {
  const index = await $axios.$get(`https://api.petexpert.pro/v1/pages/index`);
  ...
  initSlideShow(bottleType) {
   ...
   function renderBottleFrame(index, bottleType) {
    ...
    if (bottleType === "beer") {
     ...
     return require(`@/assets/img/${bottleType}-new/wa (${index + 1}).png`);
    }
   }
  },
